<template>
  <div class="page-container">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 mb-1">文章审核</h1>
        <p class="text-sm text-gray-500">管理文章内容审核流程与状态</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
      <div class="flex gap-3 flex-wrap items-center">
        <div class="relative flex-1 min-w-[200px] max-w-md">
          ...
          <input v-model="searchParams.keyword" type="text" placeholder="搜索文章标题/提交者..." class="ta-input pl-10 w-full" @input="debounceSearch" />
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">文章信息</th>
            ...
            <tr v-for="audit in audits" :key="audit.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4">
                <div>
                  ...
                  <p class="text-xs text-gray-500">ID: {{ audit.content_id }}</p>
                </div>
              </td>
      ...
      <div v-if="showDetailDialog && currentAudit" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="showDetailDialog = false">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-lg p-6 m-4">
          ...
          <div class="space-y-4">
            ...
            <div><span class="text-sm text-gray-500">内容类型:</span> <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">文章</span></div>
...
const searchParams = reactive({ keyword: '', contentType: 'article', status: '' as string | number })
...
const loadAudits = async () => {
  loading.value = true
  try {
    const params: any = { page: pagination.page, page_size: pagination.pageSize, content_type: 'article' }
